...
export class CareerPositioningWorkflow {
  ...
  private async *executeInfoCollection(
    ...
    for await (const chunk of this.agents.infoCollection.streamExecute(task)) {
      ...
      // 检查是否完成信息收集
      if (chunk.finished) {
        const nextState = this.checkInfoCollectionCompletion(
          collectedContent,
          workflowState
        );
        yield {
...
class AssessmentAgent extends AgentNode {
  ...
  async *streamExecute(task: Task): AsyncGenerator<StreamChunk> {
    try {
      ...
      if (!currentQuestion) {
        throw new Error(
          `Question not found for index: ${currentQuestionIndex}`
        );
      }
...
class AnalysisAgent extends AgentNode {
  ...
  async *streamExecute(task: Task): AsyncGenerator<StreamChunk> {
    try {
      ...
      // 调用OpenAI进行流式分析
      let analysisContent = "";
      for await (const chunk of this.openaiClient.createStreamCompletion(
        analysisMessages
      )) {
  ...
  private buildAnalysisMessages(history: OpenAIMessage[]): OpenAIMessage[] {
    ...
    messages.push({
      role: "user",
      content:
        "请基于以上信息收集和测评结果，进行综合分析。分析用户的职业倾向、优势特长、适合的职业方向，并为后续的职业推荐提供依据。",
    });
  ...
  private extractRelevantHistory(history: OpenAIMessage[]): OpenAIMessage[] {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      // 包含测评相关内容
      if (
        message.content.includes("测评问题") ||
        message.content.includes('"question"') ||
        (message.role === "user" &&
          ["a", "b", "c", "d"].includes(message.content.toLowerCase().trim()))
      ) {
        relevantHistory.push(message);
      }
    }
...
class RecommendationAgent extends AgentNode {
  ...
  async *streamExecute(task: Task): AsyncGenerator<StreamChunk> {
    try {
      ...
      // 构建推荐消息，包含历史信息和当前进度
      const recommendationMessages = this.buildRecommendationMessages(
        history,
      ...
      // 验证输出是否为有效的JSON格式
      const isValidJson = this.validateRecommendationJson(
        recommendationContent
      );

      if (!isValidJson) {
        // 如果不是有效JSON，尝试修复或重新生成
        const fallbackRecommendation = this.generateFallbackRecommendation(
          workflowState.progress
        );
        yield {
   ...
   * 构建推荐消息，包含历史信息和推荐要求
   */
  private buildRecommendationMessages(
    history: OpenAIMessage[],
    progress: number
  ): OpenAIMessage[] {
    const messages: OpenAIMessage[] = [
    ...
    const recommendationInstruction =
      this.buildRecommendationInstruction(progress);
    messages.push({
      role: "user",
  ...
  private buildRecommendationInstruction(progress: number): string {
    let instruction =
      "请基于以上信息收集、测评和分析结果，为用户推荐一个合适的职位。\n\n";
   ...
   * 生成备用推荐（当AI输出格式不正确时）
   */
  private generateFallbackRecommendation(progress: number): {
    job_title: string;
